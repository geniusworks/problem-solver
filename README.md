# <img src="assets/glyph.svg" alt="" width="34" height="32" align="top"> Problem Solver
...
The project grew out of competing in the 2024 and 2025 Advent of Code events myself: solving the
puzzles by hand led me to ask whether local models could be coordinated to solve them systematically —
and, more to the point, whether that could be *measured* rather than asserted.

> ### The quick version
>
> - **What it is** — local LLMs propose Python solutions to Advent of Code puzzles; an automatic
>   judge (an *oracle*) runs each one against the real input and checks it against the known-correct
>   answer.
> - **The finding** — letting the model take several attempts and fix its own errors **nearly
>   doubled** the solve rate on the problems it finds hardest: **42% → 75%.**
> - **The discipline** — every number here is an A/B you can re-run, recomputed from the run
>   artifacts rather than quoted from memory.
> - **The honest caveat** — the puzzles are public and pre-date the models, so a model may have
>   trained on solutions. That softens the *"how capable is the model?"* numbers. It leaves the main
>   result — *does coordinating attempts help?* — intact, because those comparisons run the same model
>   against itself: whatever it memorized, it memorized in both runs.
...
- **The model's *frontier*** — the problems it solves *sometimes but not always*. Easy ones it always
  gets; impossible ones it never does; the frontier is the uncertain band in between — where
  coordinating attempts can actually help.
- **pass@1 vs pass@k** — pass@1 is "did *one* attempt solve it?"; pass@k is "did *any* of k attempts
  solve it?" The gap between them is the room sampling has to work in.
- **Sampling / self-consistency** — draw several candidate solutions instead of one, then pick among
  them. Cures the run-to-run randomness on problems the model can already sometimes solve.
- **Self-repair** — show the model its own execution error (the traceback) and let it try again. Adds
  real information, not just another roll of the dice.
- **Contamination** — the puzzles are public and predate the models, so a model may have trained on
  solutions to them. It muddies *"how capable is the model"* but not *"does coordinating attempts
  help."*
...
On the problems the strongest model solves only *sometimes* (its own frontier, identified beforehand
by an independent classification), taking several attempts and letting each one fix its own errors
roughly doubles how many get solved:
...
that combined both. Repeated out of sample on **AoC 2025** — a year never previously measured — it
rescued exactly the uncertain problems and nothing else, which is the signature of a mechanism rather
...
Three models of the same class on the same problems, each **smaller** than the last — and the newest,
smallest one wins outright:
...
so a newer model is both plausibly stronger *and* more likely to have trained on the benchmark — the
two explanations are entangled and this work does not separate them. This is why the quick version
flags the capability numbers as descriptive. The *orchestration* findings (1 and 5) hold the model and
problems fixed and are largely unaffected. See
[RESULTS.md](RESULTS.md#training-data-contamination-the-largest-threat-to-validity).

**3. There is a hard limit, and it is not fixed by more attempts.**

One problem (2025 d9 p2) has resisted **every** configuration — **0 of 13** problem-trials across one
attempt, three attempts, with and without feedback, at two temperatures. The model repeats *the same
failing approach* each time. **Sampling multiplies draws, not diversity:** where failure is systematic
rather than random, extra attempts re-roll the same die. The obvious fixes were tested — raising
temperature and rewording the repair feedback — and both moved nothing. What *has* worked adds real
information (a fresh draw, an actual traceback); what fails adds none. This is now the standing
benchmark any "the walls can be cracked" claim has to move.
(`temperature-diversity-negative.md`, `targeted-feedback-negative.md`)
...
measurement store. All are fixed, with regression tests, and the affected numbers recomputed from the
artifacts. In a project whose premise is *measured, not asserted*, a broken instrument is the most
expensive kind of bug. (Full list in
[RESULTS.md](RESULTS.md#what-the-measurements-cost-in-credibility-and-why-that-matters).)
...
**The answer depends on which cost you actually pay:** for local GPU inference, where cost is *time*,
many cheap draws win by 36% per verified solution. For a token-metered API they lose by 3.35×. And
volume does not close the capability gap — 7/8 vs 8/8. (`economic-arm-moe-vs-generalist.md`)

**6. The structural findings generalize to a second year.**
...
verified ledger stands at **43 solutions across two years, 0 wrong.** (`generality-2025-scan.md`,
`band-2025-classification.md`)

## How it works

A model's *text* is never trusted; only code that **executes to the accepted answer** is recorded.
Self-consistency fans out several candidates; the oracle decides; a repair loop and fallback models
get extra shots before giving up.

```mermaid
flowchart TD
    A[Fetch & parse problem<br/>per-part HTML, cache offline] --> B[Analyze + pick models & strategies]
...
- **Model capability dominates on the hard problems — and it was mis-assumed.** On the never-scored
  ...
  never touches. Self-consistency fixes *variance*; a better model is what adds *capability*.
...
- **The 16 GB ceiling: two different walls, and the first diagnosis conflated them.**
  The hardest Part 2s (2024 d5 p2, d6 p2) stayed unsolved for every 16 GB model, and a 5×
  execution-timeout recovered nothing, so this read as "the model finds the right idea but writes
...
an answer separate from the solve rates, honest about what is argued versus proven.
...
1. **The pass@k-vs-pass@1 gap never closes at a model's own frontier.** Every model samples from a
   ...
   verifier turns "sometimes" into "solved". A stronger model needs fewer draws, but the gap reappears
   at *its* harder frontier — the value moves up with the model rather than vanishing.
2. **A cheap verifier turns k draws into one answer — an economic scaling law.** Where correctness is
   checkable (the oracle here, unit tests, a compiler, a proof checker), many cheap draws plus selection
...
3. **Diverse portfolios decorrelate error.** Even a frontier model has systematic blind spots; an
   ensemble of *different* models wins where their mistakes are uncorrelated.

**Arm 1 — confirmed.** The decisive test ran on `qwen3.8:27b` at its *own* measured frontier: **pass@1
42% → pass@3 75%** (`dev/progress/passk-ab-d13-d15.md`). The band was selected beforehand by an
independent 3-trial classification, and the theoretical curve was registered in writing before the k3
arm ran. (Precisely: *1 sample vs 3 samples with the repair loop held constant at 2 iterations in both
...
**Arm 3 — untested, for want of a suitable model pair, not for want of trying.** Two attempts failed
at the entry condition rather than at the mechanism: a same-tier pair whose complementary solves were
sampling noise, and a cross-generation pair whose second model solved a **strict subset** of the
first's problems (`ensemble-precheck-negative.md`, `ensemble-samp3-d4-7.md`). An ensemble can only pool
what its members *differentially* solve; neither pair had that. The entry criterion is now explicit —
measure each member individually on the targets first.

**In short:** the mechanism adds correctness at a strong model's frontier, measured rather than
argued. What is now open is sharper and more useful — **when does sampling fail, and what decorrelates
draws?**
...
3. `AOC_SESSION`: needed to **fetch** problems, inputs, and the accepted answers the oracle scores
   ...
   until you either fetch with a session cookie (see below; put it in `.env`, gitignored) or copy
   `years/` from a machine that has it. Once cached, everything runs offline. The accepted answers
   only exist for days *your account* has solved — that's what makes them ground truth.
...
<details>
<summary>Getting the AoC session cookie (Safari)</summary>

Enable the Develop menu (Settings → Advanced → "Show features for web developers"), log in to
adventofcode.com, open Web Inspector (⌥⌘I) → Storage → Cookies → adventofcode.com, and copy the
**`session`** value. It is a secret — keep it in `.env`, never commit it.
...
- **Never committed** (all of `years/` is gitignored): puzzle HTML, puzzle inputs, and the scraped
  ...
  oracle until you supply your own session cookie. (This is why `dev/verify_solutions.py` reports
  "missing input file" on a clean checkout — working as intended.)
...
- **Committed by design:** the solver's own generated Python solutions (`solutions/`), the verified
  ledger (`solutions/README.md`), and the experiment write-ups in `dev/progress/`. A few of those
  quote **short fragments of the public worked *examples*** (never real inputs) where a specific
  parsing failure has to be shown to be understood.
- **Raw run artifacts** (`dev/experiments/*.json`) are gitignored too — they can embed generated
  source and execution output. The numbers that matter are transcribed into the committed findings.

<details>
<summary>Why this is compliant with AoC's guidelines</summary>
...
|---|---|---|
| puzzle text, inputs, scraped answers | **not committed** (`years/`, gitignored) | exactly what the guideline says to exclude |
| generated solution code | `solutions/` | explicitly ours; AoC disclaims ownership of solutions |
| accepted answers | `solutions/README.md` | not covered by the request, and **per-account** — these answers cannot help anyone else, since a different account gets a different input and a different answer |
...
crashed parsing this line" is unverifiable without the line.

One timing note for anyone reusing this: publishing *current-year* solutions during the December event
cuts against community norms even where it is permitted. Everything measured here is from past years.

</details>

## What it's suited to (and what it isn't)

The testbed is AoC, but the *shape* of problem the pipeline handles well generalizes. It works best
...
- **Well-suited:** self-contained algorithmic problems with a deterministic answer and a fast check —
  parsing, simulation, small graph/grid work, arithmetic-heavy puzzles. The oracle (or example cases)
  gives an unambiguous pass/fail, so self-consistency and the repair loop have a signal to climb.
...
<details>
<summary>Established findings, and what's still open (in priority order)</summary>
...
- **A named limit**: where a model fails *systematically*, extra attempts do nothing — 2025 d9 p2 is
  0/13 across every configuration tried. Diversity, not volume, is the lever there.
- The "16 GB efficiency ceiling" turned out to be **two separate walls** — one algorithmic (a
  topological sort the earlier models never proposed), one a genuine speed limit — not the single
  phenomenon first diagnosed.

**Open, in priority order:**
- **What decorrelates draws?** The systematic-failure result says repetition is not diversity.
  Temperature, prompt variants, and model mixing are each a cheap A/B against a known-resistant
  problem.
- **The portfolio arm (arm 3):** diverse models decorrelating error — still blocked on finding a model
  pair that solves genuinely different problems.
- **A clean contamination test:** evaluating on problems published *after* the model's training cutoff
  (AoC 2026, in December) is the cheapest genuinely-unseen test available.
...
- **Hardware headroom:** these results are from one machine. Faster hardware would refine the
  capability and economic numbers (larger models fit; execution-timeout "speed walls" move) and
  unblock the tests above, but is not expected to change the orchestration mechanism, which holds the
  model and machine fixed.
