"""Targeted efficiency feedback: only on timeouts, only when enabled.
...
Gated by SolverConfig.efficiency_feedback so it can be A/B'd: a behaviour
...
class TestFeedbackGating:
    ...
    def test_enabled_adds_guidance_on_timeout(self):
        fb = _solver(efficiency_feedback=True)._build_execution_feedback(
            "m", [], [], TIMEOUT, None
        )
...
class TestConfigIsHonest:
    def test_the_flag_changes_the_fingerprint(self):
        ...
        off = SolverConfig(name="x", efficiency_feedback=False)
        on = SolverConfig(name="x", efficiency_feedback=True)
        assert off.fingerprint() != on.fingerprint()

    def test_default_is_the_historical_behaviour(self):
        assert SolverConfig(name="x").efficiency_feedback is False
