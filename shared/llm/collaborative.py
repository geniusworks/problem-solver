...
class CollaborativeImprovement:
    ...
    async def _generate_review_prompt(self, 
                                    solution: str, 
...
class EnsembleWithCollaboration:
    ...
    async def generate_and_improve(self, prompt: str) -> SolutionCandidate:
        ...
        if initial_response.error:
            raise ValueError(f"Failed to generate initial solution: {initial_response.error}")
        
        # Improve the solution collaboratively
        final_candidate = await self.collaborator.improve_solution(initial_response.content)
        
        # Validate final solution
        if not self.ensemble.validate_solution(final_candidate.solution, []):  # TODO: Add test cases
