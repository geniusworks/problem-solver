...
class ModelEnsemble:
    ...
    def __init__(
        ...
        self.health: Dict[str, ProviderHealth] = {
            provider.name: ProviderHealth(is_available=True) for provider in providers
        }
    ...
    async def _get_response_with_timeout(
        ...
        try:
            response = await asyncio.wait_for(
                provider.generate(prompt), timeout=self.timeout
            )
    ...
    async def generate_solution(self, prompt: str) -> LLMResponse:
        ...
        available_providers = [
            p for p in self.providers if self.health[p.name].is_available
        ]
        ...
        if self.strategy == VotingStrategy.QUORUM:
            ...
            required_count = max(
                2, int(len(available_providers) * self.quorum_fraction)
            )
        ...
        for provider in self.providers:
            ...
            if not health.is_available:
                ...
                response = await self._get_response_with_timeout(
                    provider, "Return 'ok' if you're working."
                )
    ...
    def _combine_responses(self, responses: List[LLMResponse]) -> LLMResponse:
        ...
        if not responses:
            return LLMResponse(
                content="", confidence=0.0, metadata={}, error="No responses received"
            )
        ...
        if self.strategy == VotingStrategy.MAJORITY:
            ...
            if len(set(r.content for r in valid_responses)) == 1:
                ...
                return LLMResponse(
                    ...
                    confidence=sum(r.confidence for r in valid_responses)
                    / len(valid_responses),
