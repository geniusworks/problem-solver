...
class RoleOptimizer:
    ...
    def _update_moving_average(self, current: float, new_value: float, weight: float) -> float:
        """Update moving average with new value."""
        return current * (1 - weight) + new_value * weight
    
    def get_optimal_roles(self, 
                         problem_type: str,
                         available_models: List[str]) -> Dict[str, List[ModelRole]]:
        ...
        assignments: Dict[str, List[ModelRole]] = {model: [] for model in available_models}
...
class AdaptiveEnsemble:
    ...
    async def solve_problem(self, problem_type: str, available_models: List[str]):
        ...
        primary_model = next(
            (model for model, roles in self.current_roles.items()
