...
class ProblemAnalyzer:
    ...
    def _analyze_examples(self, problem: ParsedProblem) -> None:
        ...
        for example in problem.examples:
            ...
            purpose = self._determine_example_purpose(
                example, prev_example, problem.description
            )
    ...
    def _determine_example_purpose(
        self, example: TestCase, prev_example: Optional[TestCase], description: str
    ) -> ExamplePurpose:
        """Determine the primary purpose of an example."""
        context = description[
            max(0, description.find(example.input_data) - 200) : description.find(
                example.input_data
            )
            + len(example.input_data)
            + 200
        ]
        ...
        if any(word in context.lower() for word in ["edge", "special", "corner"]):
            ...
            word in context.lower() for word in ["however", "but", "instead", "unlike"]
        ):
            return ExamplePurpose.NEW_CONDITION
        elif any(
            word in context.lower() for word in ["clarify", "explain", "understand"]
        ):
            return ExamplePurpose.CLARIFICATION
        elif (
            prev_example
            and len(example.input_data) > len(prev_example.input_data) * 1.5
        ):
    ...
    def _find_example_references(
        self, example: TestCase, description: str
    ) -> List[str]:
        ...
        if example_pos >= 0:
            ...
            for match in re.finditer(
                r"(?:in the example above|in this example|the example shows).*?(?:\.|$)",
                after_example,
            ):
            ...
            for line in example.input_data.splitlines():
                if line.strip():
                    for match in re.finditer(
                        f"[^0-9]{line.strip()}[^0-9]", after_example
                    ):
                        context = after_example[
    ...
    def _analyze_input_format(self, problem: ParsedProblem) -> None:
        ...
        if len(base_lines) == 1:
            ...
            if len(line_patterns) == 1:
                problem.input_format.base_format = (
                    f"repeated_pattern:{next(iter(line_patterns))}"
                )
            else:
        ...
        for example in problem.examples[1:]:
            variation = self._detect_format_variation(
                base_example.input_data, example.input_data
            )
            if variation:
    ...
    def _detect_format_variation(
        self, base_input: str, new_input: str
    ) -> Optional[str]:
        ...
        base_seps = (
            set(re.findall(r"[^a-zA-Z0-9\s]", base_lines[0])) if base_lines else set()
        )
        new_seps = (
            set(re.findall(r"[^a-zA-Z0-9\s]", new_lines[0])) if new_lines else set()
        )
    ...
    def _detect_condition_changes(self, problem: ParsedProblem) -> List[str]:
        ...
        for example in problem.examples:
            for ref in example.referenced_by:
                conditions = re.findall(
                    r"must|should|needs to|has to|cannot|can\'t", ref, re.IGNORECASE
                )
                example_conditions.update(conditions)

        # Extract conditions from parts mentioning full input
        full_input_contexts = re.finditer(
            r"(?:actual|full|real).*?input.*?(?:\.|$)",
            problem.description,
            re.IGNORECASE | re.DOTALL,
        )
        for match in full_input_contexts:
            conditions = re.findall(
                r"must|should|needs to|has to|cannot|can\'t",
                match.group(0),
                re.IGNORECASE,
            )
        ...
        if full_conditions - example_conditions:
            changes.append(
                f"Additional conditions in full input: {', '.join(full_conditions - example_conditions)}"
            )
    ...
    def categorize_problem(self, problem: ParsedProblem) -> List[ProblemCategory]:
        ...
        for category, indicators in self.category_indicators.items():
            if any(
                indicator.lower() in problem.description.lower()
    ...
    def determine_input_format(self, problem: ParsedProblem) -> Optional[str]:
        ...
        return (
            problem.input_format.base_format
            if hasattr(problem, "input_format")
            else None
        )
